...
class Extractor:
    ...
    def binary_string_to_bytes(self, bin_str): 
        return int(bin_str, 2).to_bytes((len(bin_str) + 7) // 8, byteorder='big')

    def extract_metadata(self):  #pengecekan byte didalam bit sampai dapat # untuk panjang_data kemudian mendapatkan ekstensi # setalah itu data asli pesan dalam byte
        num_of_sharp = 0
        metadata = []
        for i in range(len(self.string_message)):
            if (self.string_message[i]).decode('utf-8') == '#':
                metadata.append(i)
                num_of_sharp += 1
            if num_of_sharp == 2:
                break
        panjang_data = int(b''.join(self.string_message[0:metadata[0]]).decode('utf-8'))
        extension = b''.join(self.string_message[metadata[0]+1:metadata[1]]).decode('utf-8')
        rest = bitstring.BitArray(b''.join(self.string_message[metadata[1]+1:metadata[1]+1+panjang_data]))
        self.write_to_file("Data Ekstrak", rest.bin)
        print(panjang_data, extension)
        return panjang_data, extension, bitstring.BitArray(bin=rest.bin[:panjang_data]).bytes
                                ...
                                for j in range(8):
                                    if i > 0 or j > 0:
                                        ...
                                        if index % mod_index != (mod_index - 1):
                                            ...
                                            temp += str(extracted_bit)
                                            message.append(self.binary_string_to_bytes(temp))
                                            temp = ""
                                        index += 1
    ...
    def parse_message(self):
        self.len_message, self.extension, self.string_message = self.extract_metadata()
